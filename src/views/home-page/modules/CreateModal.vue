<template>
  ...
  >
    ...
    >
      ...
      <a-form-model-item label="英文标题" prop="titleEn" v-else>
        <a-input v-model="form.titleEn" placeholder="英文标题"/>
      </a-form-model-item>
      <a-form-model-item label="描述" prop="description" v-if="tabkey==1">
        <a-input v-model="form.description" placeholder="描述" />
      </a-form-model-item>
      <a-form-model-item label="英文描述" prop="descriptionEn" v-else>
        <a-input v-model="form.descriptionEn" placeholder="英文描述" />
      </a-form-model-item>
  ...
  methods: {
    ...
    onSubmit () {
      this.$refs.ruleForm.validate(vaild => {
        if(vaild) {
          if(this.status == 1) {
            addHomePageInfo(this.form).then(res => {
              if(res.code == '0') {
                this.showMessage(res,() => {
                  this.onClose()
                  this.parent.getList()
            ...
            updateHOmePageList({id:this.id,...this.form}).then(res => {
              if(res.code == '0') {
                this.showMessage(res,() => {
                  this.onClose()
                  this.parent.getList()
